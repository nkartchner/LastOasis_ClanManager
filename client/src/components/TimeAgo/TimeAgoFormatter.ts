import { TimeagoIntl, IL10nsStrings } from "./TimeAgoIntl";
...
export class TimeagoCustomFormatter extends TimeagoFormatter {
  ...
  private parse(
    ...
    if (
      unit === "week" &&
      !this.intl.strings.week &&
      !this.intl.strings.weeks
    ) {
    ...
    const pluralUnit: any = unit + "s";
    const pluralKey: keyof IL10nsStrings = pluralUnit;
    if (isPlural) {
      const stringFn =
        this.intl.strings[pluralKey] || this.intl.strings[unit] || "%d " + unit;
      dateString.push(normalize(stringFn as StringOrFn));
    } else {
      const stringFn =
        this.intl.strings[unit] || this.intl.strings[pluralKey] || "%d " + unit;
      dateString.push(normalize(stringFn as StringOrFn));
    }
    ...
    const wordSeparator =
      typeof this.intl.strings.wordSeparator === "string"
        ? this.intl.strings.wordSeparator
        : " ";
  ...
  private normalizeNumber(numbers: NumberArray, value: number) {
    return numbers && numbers.length === 10
      ? String(value)
          .split("")
          .map((digit: string) =>
            digit.match(/^[0-9]$/) ? numbers[parseInt(digit, 10)] : digit
          )
          .join("")
      : String(value);
  }
    ...
    return (stringOrFn: StringOrFn) =>
      typeof stringOrFn === "function"
        ? stringOrFn(value, millisDelta).replace(
            /%d/g,
            this.normalizeNumber(numbers!, value)
          )
