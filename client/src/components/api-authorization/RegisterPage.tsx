...
const Register: React.FC<RouteComponentProps> = ({ history }) => {
  ...
  const [lastName, setLastName] = React.useState<string>(faker.name.lastName());
    ...
    Axios.post<{ user: User }>("/api/auth/register", {
      ...
      .catch((err: AxiosError) => {
        ...
        if (errors) {
          ...
          if ("ConfirmPassword" in errors) setPwError(errors.ConfirmPassword);
        }
  ...
  return (
    <Grid container direction="column" justify="center" alignItems="center">
      ...
      <Grid item xs={12}>
        ...
        >
          {isRegistering ? <CircularProgress color="primary" /> : "Submit"}
        </Button>
      </Grid>
    </Grid>
  );
