...
const NewClanForm: React.FC<RouteChildrenProps> = ({ history }) => {
  ...
  const handleSubmit = () => {
    Axios.post<Clan>("/api/clan/new", {
      ...
      .then(({ data }) => {
        dispatch(
          UserActions.updateUser({ ...user!, clanId: data.id, role: "Founder" })
        );
        history.push(`/clans/${data.id}/posts`);
      })
  ...
  const LeaveClanWarning = (
    <>
      <Typography className={classes.warningText} variant="h3" color="error">
        Warning!
      </Typography>
      <Typography
  ...
  const Form = (
    <>
      ...
      >
        {FLAGS.map((flag, i) => (
          <MenuItem key={flag.id} className={classes.colorBox} value={flag.src}>
            <img
              src={flag.src}
              style={{
